...
describe("GET /api/packages/:id/dependents (A5)", () => {
  beforeEach(() => {
    createClientMock.mockReset();
  });

  it("returns a ListResponse<DependentRow>", async () => {
    const rows = [
      {
        from_package_id: "a",
        from_package: "body-parser",
        from_version: "1.20.0",
        dep_kind: "dependency",
      },
    ];
    ...
    const { GET } = await import("@/app/api/packages/[id]/dependents/route");
    const res = await GET(req(`/api/packages/${PKG_ID}/dependents`), {
  ...
  it("returns 400 for bad UUID", async () => {
    const { GET } = await import("@/app/api/packages/[id]/dependents/route");
    const res = await GET(req(`/api/packages/bad/dependents`), {
